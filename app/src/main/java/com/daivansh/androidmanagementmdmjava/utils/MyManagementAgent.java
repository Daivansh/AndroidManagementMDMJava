...
public class MyManagementAgent {

    /** The Android Management API client. */
    private final AndroidManagement androidManagementClient;

    public MyManagementAgent(AndroidManagement androidManagementClient) {
...
//        setPolicy(ENTERPRISE, POLICY_ID, getCosuPolicy());

        // Create an enrollment token to enroll the device.
//        String token = createEnrollmentToken(ENTERPRISE, POLICY_ID);
//        System.out.println("Enrollment token (to be typed on device): " + token);
    ...
    public static AndroidManagement getAndroidManagementClient(Context context)
            throws IOException, GeneralSecurityException {
        InputStream inputStream=context.getAssets().open("AndroidManagement1.json");
        GoogleCredential credential = GoogleCredential.fromStream(inputStream)
                .createScoped(Collections.singleton(StaticConstants.OAUTH_SCOPE));
                ...
                .setApplications(
                        Collections.singletonList(
                                new ApplicationPolicy()
                                        .setPackageName(StaticConstants.COSU_APP_PACKAGE_NAME)
                ...
                .setPersistentPreferredActivities(
                        Collections.singletonList(
                                new PersistentPreferredActivity()
                                        .setReceiverActivity(StaticConstants.COSU_APP_PACKAGE_NAME)
                                        .setActions(
    ...
    public Policy setPolicy(Policy policy)
            throws IOException {
        System.out.println("Setting policy...");
        String name = StaticConstants.ENTERPRISE + "/policies/" + StaticConstants.POLICY_ID;
        return androidManagementClient
                .enterprises()
                .policies()
                .patch(name, policy)
    ...
    public EnrollmentToken createEnrollmentToken(String duration)
            throws IOException {
        System.out.println("Creating enrollment token...");
        EnrollmentToken token =
                new EnrollmentToken().setPolicyName(StaticConstants.POLICY_ID).setDuration(duration);
        return androidManagementClient
                .enterprises()
                .enrollmentTokens()
    ...
    public Empty deleteDevice(String deviceName,List<String> wipeDataFlags) {
